...
/* Aplica um estimulo e registra o ciclo completo de clock (0 -> 1) */
void aplicar(int va, int vsel0, int vsel1, int vcin){
    a = va; sel0 = vsel0; sel1 = vsel1; cin = vcin;

    /* --- Fase 1: clk=0, aplica novas entradas --- */
    clk = 0;
    ...
    if (registrador_valido){
        /* s/cout ainda refletem o valor antigo do acumulador aqui */
    ...
    clk = 1;
    adac_4(a, sel0, sel1, cin, clk); /* atualiza acc/cout no modelo comportamental */
    ...
    /* --- Fase 3: checa s/cout ja com folga para o clock-to-Q --- */
    if (registrador_valido){
    ...
    /* --- Fase 4: derruba o clock antes do proximo vetor --- */
    clk = 0;
    AFFECT(inttostr(tempo), "clk", inttostr(clk));
    last_clk = clk;
...
main() {
    ...
    DEF_GENPAT("i_adac_4_delay");
    SETTUNIT("ns"); //mudar a escala para nanosegundos
